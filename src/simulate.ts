import type { Address, GetCodeParameters } from "viem";
...
import {
  ...
  discoverBalanceSlot,
  type AllowanceSlot,
  type TokenBalanceSlot,
} from "./internal/probes.js";
...
export async function simulate(args: SimulateArgs): Promise<SimulationResult> {
  ...
  const balanceOverrides = balanceSlots.map((slot) =>
    storageOverride(slot.token, slot.slot, OVERRIDE_TOKEN_AMOUNT),
  );
  ...
  const highAllowanceOverrides = allowanceSlots.map((slot) =>
    storageOverride(slot.token, slot.slot, OVERRIDE_TOKEN_AMOUNT),
  );
  const withAllowances = await runWithOverrides(args, calls, candidateAddresses, [
    ...balanceOverrides,
    ...highAllowanceOverrides,
  ]);
...
function candidateSpenders(
  ...
  return uniqueAddresses([...calls.map((call) => call.to), ...candidateAddresses]).filter(
    (address) => {
...
function buildGetCodeParameters(
  args: { address: Address } & Pick<SimulateArgs, "blockNumber" | "blockTag">,
): GetCodeParameters {
  return (
