...
import { buildStateOverride, storageOverridesToStateDiff, type StateOverrideEntry, type StorageOverride } from './stateOverride.js';
...
export async function runSimulator(args: {
  ...
  debugStep?: string;
} & BlockOptions): Promise<InternalSimulationResult> {
  ...
  const failingCallIndex = result.failingCallIndex === (1n << 256n) - 1n ? undefined : Number(result.failingCallIndex);
