...
describe('viem-tx-sim', () => {
  ...
  it.only('reports native value deltas', async () => {
  ...
  it('discovers allowance gaps from token outflow and attributes the spender', async () => {
    ...
    expect(events.filter((event) => event.step === 'txSimulator.simulate' && event.phase === 'start')).toHaveLength(3);
  });
  ...
  it('keeps batch state changes visible between calls', async () => {
    ...
    expect(result.assetBalanceDeltas.some((delta) => delta.asset !== 'native' && delta.delta < 0n && delta.spender !== undefined)).toBe(false);
  });
  ...
  it('verifies proxy token storage slots before overriding balances', async () => {
    const implementation = await deploy('TestToken.sol', 'TestToken', ['Implementation', 'IMPL', 18]);
    ...
    const hash = await (ctx.walletClient as any).deployContract({
  ...
  async function write(contract: { abi: any; address: `0x${string}` }, functionName: string, args: readonly unknown[] = []) {
    const hash = await (ctx.walletClient as any).writeContract({
